...
class MockServer {
  ...
  start() {
    ...
    this.app.get('*', (req, res, next) => this.process(req, res, next));
  ...
  process(req, res, next) {
    ...
    switch(info.type) {
      ...
      case 'proxy': 
        return this.handleProxy(info, req, res, next);
      case 'api':
      default:
  ...
  handleStatic(info, req, res, next) {
    ...
    if (fs.existsSync(fullpath)) {
      ...
      if (this.config.verbose) {
        console.log({ time: new Date().toISOString(), api: req.path, static: info.src });
      }
    } else {
  ...
  handleApi(info, req, res, next) { 
    ...
    if (!fs.existsSync(fullpath)) {
      this.handleError(res, 404, 'API file not found', { api: req.path });
      return;
    }
  ...
  handleProxy(info, req, res, next) { 
    ...
    this.log({ api: req.path, proxy: info.src });
